...
# `mocker` - это специальный объект от pytest-mock, который мы получаем как аргумент
def test_get_top_customers_data_returns_correct_data(mocker):
    ...
    # mocker.patch() находит объект по указанному пути и заменяет его.
    mock_execute_query = mocker.patch(
        # Путь к функции, которую подменяем: 'имя_файла.имя_функции'
        'payment_analyzer.execute_query',
        # Говорим, что при вызове эта подделка должна вернуть наши данные.
